    ...
    <div aria-labelledby="auth-modal-title" aria-modal="true" className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4" role="dialog">
      <section className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          ...
          <button aria-label="Close sign in dialog" className="rounded-lg px-2 py-1 text-slate-500 hover:bg-slate-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-600" onClick={onClose} type="button">×</button>
        </div>
        <div aria-label="Authentication mode" className="mt-6 grid grid-cols-2 rounded-lg bg-slate-100 p-1" role="tablist">
          {(['login', 'signUp'] as const).map((nextMode) => (
            <button
              ...
              className={`rounded-md px-3 py-2 text-sm font-semibold transition ${mode === nextMode ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600'}`}
              key={nextMode}
        ...
        <form className="mt-6 space-y-4" noValidate onSubmit={handleSubmit}>
          ...
          <button className="w-full rounded-lg bg-amber-500 px-4 py-2.5 font-semibold text-slate-950 transition hover:bg-amber-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-600 disabled:cursor-not-allowed disabled:bg-slate-200" disabled={loading} type="submit">
